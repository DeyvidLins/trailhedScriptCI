...
def print_content_tag(nome_arquivo, tag_parent):
    dic_tags = {}
    ...
    for key in dic_tags.keys():
        if key != '@xmlns':
            build_file(key,dic_tags[f'{key}'])
         

def build_file(tag_parent,body_tag):
    ...
    diff_tag_component.add_new_tag_component(tag_parent,new_tag_component)
